...
La comunidad autónoma españolas con menor satisfacion es **Ceuta** con 6.3.
...
```
    ...
    La satifacción con el entrono en Aragón son ligeramente superiores a la media, 7.6.
...
```
    ...
    La satifacción con el entrono en Región de Murcia, son ligeramente por debajo de la media, 7.
...
```
    ...
    La satifacción con el entrono en Comunidad de Madrid son ligeramente por debajo de la media, 7.1.
...
* **Conclusiones sobre de la satisfacción con el entorno según la comunidad**

Basándonos en los datos obtuvimos mediante diversas operaciones sobre el set llamado Satisfacción del entorno en la vida, en este caso. 
Podemos afirmar que la comunidad más satisfecha es la Comunidad Foral de Navarra. Lo cual puede ser consecuencia de diversos factores, como los recursos, la economía, el clima, la sociedad… de dicha comunidad autónoma. 

Por otro lado, la comunidad menos satisfecha con su entorno es Ceuta. La cual, puede deberse como en el caso anterior a diversos factores, pero también cabe a destacar que se trata de una de las ciudades autónomas más pequeñas del país.
...
ggplot(data = Discapacitados, aes(x = `Comunidades`, y = `Personas con discapacidad`)) +  geom_bar(stat="identity", position="stack", colour="yellow", show.legend = TRUE) + ggtitle("Análisis de la contaminación") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
También es conveniente que todos los nombres de las columnas   cumplan las reglas sintácticas. En este caso todos los datos de
dichas columnas son de tipo "double".
...
Basándonos en los datos obtenidos con el análisis, podemos ver que la de dificultades para pagar las cosas esenciales es de un valor de 41.46 puntos. Pero al ser el mínimo 26.40 y el máximo 52.92, vemos que ambos valores no son proximos al de la media. Por ello, podemos afrimar que hay bastante variación en todo el país. Cabe destacar que los que tienen la mayor dificulta para pagar los gastos esenciales son los los habitantes de Andalucia y por otro lado lo que menos son los del País Vasco. Con esto volvemos a remarcar la diferencia significativa que hay entre el sur y el norte del país, tal y como hemos comentando en otros análisis.
...
```
En el caso del análisis de los ingresos limitados el valor medio es de 34.28 y tiene unos máximos y mínimos bastantes alejados de la media. Pudiendo así afirmar que los valores son basnatante dispares en todo el estado. En el caso de esta variable también, los discapacitados con menos ingresos son viven en Andalucia y los de mayor ingresos en el País Vasco. Volviendo a destacar la diferencia entre el sur y el norte del país.
...
Con respecto a las dificultades de gestionar el dinero, obtenemos que la media es del 0.42. Y en este caso los minimos y másximos son muy dispares, sobre todo en el caso del máximo. Tiene un valor de 1.84 y corresponde al Principado de Asturias.
...
```
Analizando esta última variable, que nos indica los coses qeu tienen qeu asumir por la propia enfermedad hemos obtenido que la media es de 1.210(son euros mensuales). Es destacable que el mínimo es de 0€, por lo que asumimos que el las ciudades autonomas de Murcia y Melilla, tanto con ayudas o subenciones todo el gasto referido a su enfermedad o problema crónico no son asumidos por el afectado. Por otro lado tenemos en el másximo a la Región de Murcia don 2.110€, por lo que concluimos qeu es la comunidad con menos ayudas económicas dirigidas a personas con discapacidad. En el gráfico se aprecia también, que exceptuando los dos extremos indicados anteriormente, la diferencia entre comunidades no es extremadamente diversas, siempre hablando de forma generalizada.

* **Conclusiones sobre la economía de las personas con discapacidad**

Aunque hayamos análizado variables completamente diferentes sobre algunos aspectos de la economia de una persona discapacida, podemos sacar una conclusión generalizada de dicha economia a grandes ragos.

Por un lado, es clara la diferneciación a nivel economico en este caso de las personas discapacitadas dependiendo de la comunidad en la que vivan, sobre todo es bastante visible la diferencia entre las comunidades situadas al sur del país y al norte.Ya que en general las del norte tiene mejor economia y menos gastos en el caso de las personas discapacitadas (con la excepción de Ceuta y Melilla).

También es muy destacable en qué comunidades dan mayores facilidades para las personas con discapacidad a nivel financiero. Y es curioso el gran salto que hay entre comunidades de un mismo país. Teniendo en cuenta que la economía de un discapacitado es significativamente relevante para tener una mejor calidad de vida, esto influye en gran medida a su vida en general.


### 3. Relación entre el progreso de las discapacidades y el entorno

#### Número de centros de salud por comunidad.
```{r}
library(readxl)
...
```
Este set de datos tiene 18 filas y 5 columnas con información sobre las el número de centros de salud o consultorios locales de las comunidades autónomas.
Las 18 columnas se deben a que España esta formado por 17 ciudades autónomas y dos ciudades autónomas las cuales ocupan en este set de datos la misma fila.
...
[Fauna silvestre y accidentes de tráfico en Asturias](https://repositorio.unican.es/xmlui/handle/10902/17202/ "Fauna silvestre y accidentes de tráfico en Asturias").
...
* **Conclusiones sobre las limitaciones en actividades básicas**
...
También es conveniente que todos los nombres de las columnas cumplan las reglas sintácticas. En este caso todos los datos dedichas columnas son de tipo "double".
...
* **Conclusiones sobre los Cuidadores de las personas con discapacidad**

Como hemos análizado tres tipos de cuidadores, mediante la comparación de las medias podemos determinar que hay más discapacitados viven en el hogar de la persona que los cuida que los que viven en otros hogares, siempre teniendo encuenta el posible margen de error que podrían causar los que no constan. Cabe a destacar el caso de Andalucía, por sus valores tan significativos y dispares al resto de las comunidades. En las conclusiones finales haremos un join para relacionar el set de los cuidadores junto con el de la economia y el de ayudas, ya que creemos que la relación entre los tres factores puede ser interesante en nuetra investigación.
...
* [Caso Andalucia](https://www.eldiario.es/andalucia/anos-espera-reconocimiento-discapacidad-andalucia_1_8461860.htmll/ "Caso Andalucia")
...
```{r}
library(readxl)
Barreras_en_la_participación_social <- read_excel("DATA/Barreras en la participación social.xlsx")
View(Barreras_en_la_participación_social)
```

**2.** Vamos a comprobar algunas medidas estadísticas con la información recogidos en las distintas columnas

```{r}
dim(Barreras_en_la_participación_social)

class(Barreras_en_la_participación_social)

```

El set de datos es un tipo de data frame conocido como **tibble**.

Además sabemos que el tibble contiene **19 filas y 11 columnas**, esto es así porque España se compone de 17 comunidades autónomas y 2 cuidades autónomas.
...
```{r}
summary(Barreras_en_la_participación_social, na.rm = TRUE)

Media_SalirDeCasa <- mean(as.double(Barreras_en_la_participación_social$`Salir de casa`), na.rm=TRUE)

Media_UsoTransporte <- mean(as.double(Barreras_en_la_participación_social$`Uso del transporte`), na.rm=TRUE)

Media_AccesoEdificios <- mean(as.double(Barreras_en_la_participación_social$` Acceso y movilidad por los edificios `), na.rm=TRUE)

Media_ActFormativas <- mean(as.double(Barreras_en_la_participación_social$`Actividades formativas`), na.rm=TRUE)

Media_Empleo <- mean(as.double(Barreras_en_la_participación_social$`Acceso a un empleo adecuado`), na.rm=TRUE)

Media_UsoInternet <- mean(as.double(Barreras_en_la_participación_social$`Uso de internet`), na.rm=TRUE)

Media_ApoyoSocial <- mean(as.double(Barreras_en_la_participación_social$` Contacto y apoyo social `), na.rm=TRUE)

Media_Ocio <- mean(as.double(Barreras_en_la_participación_social$`Actividades de ocio y culturales`), na.rm=TRUE)

Media_SitEconomica <- mean(as.double(Barreras_en_la_participación_social$`Situación económica`), na.rm=TRUE)

Media_Discriminacion <- mean(as.double(Barreras_en_la_participación_social$`Discriminación`), na.rm=TRUE)

Medias <- c(Media_SalirDeCasa, Media_UsoTransporte, Media_AccesoEdificios, Media_ActFormativas, Media_Empleo, Media_UsoInternet, Media_ApoyoSocial, Media_Ocio, Media_SitEconomica, Media_Discriminacion)

max(Medias, na.rm=TRUE)

```
Calculando la media de los valores de todas las comunidades de cada tipo de barreras en la participación social hemos obtenido un resultado 71.18%. Esto es, podemos afirmar que las barreras socuales que sifren las paersonas discapacitadas, teniendo en cuenta diferentes tipos de barreras, a nivel estatal estas barreras afectan al casi 3 de cada 4 personas discapacitadas.

* **Análisis estadístico por columnas**

```{r}
summary(Barreras_en_la_participación_social $` Salir de casa`, na.rm = TRUE)

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Salir de casa`)== min(as.double(Barreras_en_la_participación_social $`Salir de casa`), na.rm = TRUE)]

Barreras_en_la_participación_social$`Comunidad`[as.double (Barreras_en_la_participación_social$`Salir de casa`) == max(as.double(Barreras_en_la_participación_social $`Salir de casa`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Salir de casa`)) +  geom_bar(stat="identity", position="stack", fill="pink") + ggtitle("Salir de casa") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a la barrera social de salir de casa Castilla y leon  presenta el índice más bajo, mientras que Andalucía es la que tiene mayor proporción. Esto podría deberse al clima, ya que en Andalucía el calor constante que hace durantes la mayoria de los meses, podría incomodar el salir de casa. Y por lo contraría como el clima de Castilla y Leon es más cambiante y varia más la temperatura sería más agradable en este caso.

```{r}
summary(Barreras_en_la_participación_social $`Uso del transporte`, na.rm = TRUE)

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Uso del transporte`)== min(as.double(Barreras_en_la_participación_social $`Uso del transporte`), na.rm = TRUE)]

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social$`Uso del transporte`) == max(as.double(Barreras_en_la_participación_social$`Uso del transporte`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Uso del transporte`)) +  geom_bar(stat="identity", position="stack", fill="green") + ggtitle("Uso del transporte") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a la barrera del uso del transporte la región de Murcia presenta el índice más bajo, mientras que el País Vasco y Ceuta son las que tienen mayor proporción. Esto esta completamente ligadoa la calidad y vehiculos adaptados que tengan en cada comunidad, por ello, podemos afrimar que tanto el País Vaco como Ceuta tendrán el transporte público más comodo y adaptado a las personas discapacitadas.


```{r}
summary(Barreras_en_la_participación_social $`Acceso y movilidad por los edificios`, na.rm = TRUE)

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Acceso y movilidad por los edificios`)== min(as.double(Barreras_en_la_participación_social $`Acceso y movilidad por los edificios`), na.rm = TRUE)]

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Acceso y movilidad por los edificios`) == max(as.double(Barreras_en_la_participación_social $`Acceso y movilidad por los edificios`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Acceso y movilidad por los edificios`)) +  geom_bar(stat="identity", position="stack", fill="blue") + ggtitle("Acceso y movilidad por los edificios") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a la barrera del acceso y movilidad por los edificios Canarias presenta el índice más bajo, mientras que la Comunidad Foral de Navarra es la que tiene mayor proporción. Este aspecto esta completamente relacionado con las instalacines que puedan tener cada comunidad en sus edificios, en este caso, instalaciones para facilitar el paso y la movilidad de las personas discapacitadas. Por lo que, Navarra tendrá las mejores instalaciones para las personas discapacitadas den sus edificios. 

```{r}
summary(Barreras_en_la_participación_social $`Actividades formativas`, na.rm = TRUE)

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Actividades formativas`)== min(as.double(Barreras_en_la_participación_social $`Actividades formativas`), na.rm = TRUE)]

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Actividades formativas`) == max(as.double(Barreras_en_la_participación_social $`Actividades formativas`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Actividades formativas`)) +  geom_bar(stat="identity", position="stack", fill="red") + ggtitle("Actividades formativas") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a las comunidades con barreras en las actividades formativas para personas con discapacidad canarias vuelve presenta el índice más bajo, mientras que la Comunidad Foral de Navarra es la que tiene mayor proporción. Las actividades formativas que pueda tener cada comunidad se basa en la oferta de actividades que tengan en cada comunidad. Y por ello, podemos afirmas que Navarra vuelve a destacar con la mayor oferta diriguida almenos a las personas discapacitadas o loque es lo mismo con el mínimo ed barreras sociales en el ambito de las actividades formativas. 

```{r}
summary(Barreras_en_la_participación_social $`Acceso a un empleo adecuado`, na.rm = TRUE)

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Acceso a un empleo adecuado`)== min(as.double(Barreras_en_la_participación_social $`Acceso a un empleo adecuado`), na.rm = TRUE)]

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social$`Acceso a un empleo adecuado`) == max(as.double(Barreras_en_la_participación_social$`Acceso a un empleo adecuado`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Acceso a un empleo adecuado`)) +  geom_bar(stat="identity", position="stack", fill="green") + ggtitle("Acceso a un empleo adecuado ") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a las barreras en el acceso a un empleo adecuado la Comunidad Foral de Navarra vuelve a presentar el índice más bajo, mientras que en este caso Ceuta es la que tiene mayor proporción. Lo que indicaría que Navarra en la comunidad con menos barreras sociales para las persoans discapacitadas encuanto a acceder a un empleo adecuado. En este caso puede deverse a diversos factores, como la facilidad a solicitar dichos trabajos, guardar plazas específicas para las personas con dificultades como estas, subenciones para que las empresas puedan ofrecer trabajos a personas con discapacidad...

```{r}
summary(Barreras_en_la_participación_social $`Uso de internet`, na.rm = TRUE)

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Uso de internet`)== min(as.double(Barreras_en_la_participación_social $`Uso de internet`), na.rm = TRUE)]

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social$ `Uso de internet`) == max(as.double(Barreras_en_la_participación_social$`Uso de internet`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Uso de internet`)) + geom_bar(stat="identity", position="stack", fill="green") + ggtitle("Uso de internet") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a las barreras del uso de internet el País Vasco presenta el índice más bajo, mientras que Melilla es la que tiene mayor proporción. En conclisión el País vasco será la comuidad con menos barreras para que las personas con dicapacidad puedan usar el internet como cualquier otra persona. Esto puede deberse a algún tipo de ayuda que pueda tener la comunidad para evitar este tipo de dificultades en sus vidas.

```{r}
summary(Barreras_en_la_participación_social $`Contacto y apoyo social`, na.rm = TRUE)

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Contacto y apoyo social`)== min(as.double(Barreras_en_la_participación_social $`Contacto y apoyo social`), na.rm = TRUE)]

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social$`Contacto y apoyo social`) == max(as.double(Barreras_en_la_participación_social$`Contacto y apoyo social`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Contacto y apoyo social`)) +  geom_bar(stat="identity", position="stack", fill="purple") + ggtitle("Contacto y apoyo social ") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a las barreras en el contacto y apoyo social Extremadura presenta el índice más bajo, mientras que Melilla es la que tiene mayor proporción.

```{r}
summary(Barreras_en_la_participación_social $`Actividades de ocio y culturales`, na.rm = TRUE)

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Actividades de ocio y culturales`)== min(as.double(Barreras_en_la_participación_social $`Actividades de ocio y culturales`), na.rm = TRUE)]

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social$`Actividades de ocio y culturales`) == max(as.double(Barreras_en_la_participación_social$`Actividades de ocio y culturales`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Actividades de ocio y culturales`)) +  geom_bar(stat="identity", position="stack", fill="blue") + ggtitle("Actividades de ocio y culturales ") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a las barreras que pueden ocasionar las actividades de ocio y culturales Melilla presenta el índice más bajo, mientras que Andalucía es la que tiene mayor proporción. La facilidad para que las personas con discapacidad puedan realizar actividades de ocio y culturales, puede deverse a diversos factores, entre ellos, las subenciones que puedan obetener, las instalaciones de los recintos en los que se vayan a celebrar dichas actividades ect. Y grácias a los datos, podemos saber que Melilla será quine más facilidades les de en este ambito, disminuyendo así la barrera social.

```{r}
summary(Barreras_en_la_participación_social $`Situación económica`, na.rm = TRUE)

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Situación económica`)== min(as.double(Barreras_en_la_participación_social $`Situación económica`), na.rm = TRUE)]

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social$`Situación económica`) == max(as.double(Barreras_en_la_participación_social$`Situación económica`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Situación económica`)) +  geom_bar(stat="identity", position="stack", fill="yellow") + ggtitle("Situación económica") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a las barreras en la situación económica el País Vasco presenta el índice más bajo, mientras que Andalucía es la que tiene mayor proporción. Esto es, el País Vasco es la comunidad que más ayuda o se implica en la situación económica de las personas con dicapacidad.

```{r}
summary(Barreras_en_la_participación_social $`Discriminación`, na.rm = TRUE)

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social $`Discriminación`)== min(as.double(Barreras_en_la_participación_social $`Discriminación`), na.rm = TRUE)]

Barreras_en_la_participación_social$`Comunidad`[as.double(Barreras_en_la_participación_social$`Discriminación`) == max(as.double(Barreras_en_la_participación_social$`Discriminación`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Discriminación`)) +  geom_bar(stat="identity", position="stack", fill="white") + ggtitle("Discriminación ") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a las barreras sociales en la discriminación Galicia presenta el índice más bajo, mientras que la Rioja es la que tiene mayor proporción.


* **Conclusiones barreras en la participación social**

Observando todos los datos obtenido y sobre todo las gráficas de cada tipo de barrera social. Hemos podido observar qeu a pesar de que las barreras sociales en general sean muy similares en todas las comunidades analizadas, siemrpe hay alguna comunidad que destaca con el bajo indice de dichas barreras, lo cual indica qeu si otras comunidades han podido bajar ese indice en general todas las comunidades podrian llegar al mismo nivel. Evitando así o, al menos, disminuyendo las barreras sociales con las que tienen que lidiar las personas con dicapacidad. Sabiendo que su día a día ya de por si es más compleja que el de resto de personas, las barreras sociales tendrían que ser mínimas en todas las comunidades, no ser un impedimento más en su vida diaria.
...
El mínimo corresponde a la ciudad de Ceuta. 
El máximo corresponde a la ciudad de Melilla.
...
El mínimo corresponde a la comunidad de Asturias en esto puede influir que Asturias tiene el menor dato demográfico. 
El máximo corresponde a la ciudad de Ceuta.Esta ciudad encabeza limitaciones para ocuparse de los trámites administrativos diarios, con mayor insatisfación con el entorno. Este dato choca bastante con que en el análisis anterior hemos comprado que la ciudad/comunidad autónoma con menor indice de discapacidad en hombres es Ceuta esto puede deberse a la discriminación por sexo que sufren las mujeres y no los hombres.  

#### Conclusiones relación entre la discapacidad y el entorno en el que viven.

1. La calidad de vida, la satisfacción con el entorno y la esperanza de vida es mayor en comunidades con **mayor poder adquisitivo**, generalmente situadas en el norte de España. 
...
![](C:\Users\usuario\Documents\INGENIERIA DE LA SALUD\3º UNI GIS\FUENTES DE DATOS BIOMEDICAS Y WEB SEMANTICAS\El precio de la discapacidad\El-precio-de-la-discapacidad\IMAGES\satisfaccion_calidad_vida.png)

2. La **proporción de discapacitados** no presenta diferencias entre pueblos y ciudades, pero la gravedad y la asistencia clínica de las discapacidades si es diferente en función de la comunidad. La diferencia proviene mayoritariamente de la **calidad del servicio sanitario**, siendo la mejor de todas Navarra. Por ello, podemos sacar como conclusión la idea de que existe una relación entre la gravedad de las patologías y la economía de los discapacitados.
...
3. Anteiormente veaíamos que la mayor parte de los discapacitados sufren problemas a la hora de desempeñar las tareas cotidianas del hogar. Pues bien, a continuación vamos a ver que estas limitaciones también están relacionadas con el nivel de **discriminación social** que sufren los discapacitados. Que una persona no puede completar tareas domésticas básicas refleja los múltiples problemas que padecerá para moverse, y, por tanto, lo limitada que se verá su vida social al no poder realizar todo tipo de planes con su círculo más cercano. 
...
![](C:\Users\usuario\Documents\INGENIERIA DE LA SALUD\3º UNI GIS\FUENTES DE DATOS BIOMEDICAS Y WEB SEMANTICAS\El precio de la discapacidad\El-precio-de-la-discapacidad\IMAGES\depresion.png)
...
4. No apreciamos ninguna relación considerable entre la satisfacción de las personas con el entorno en el que viven y las necesaidades sanitarias no satisfechas que contemplan. Hemos hecho una unión entre las dos tablas que tratan estos temas y no hay ningún patrón apreciable. Inicialmente sospechabamos que cuanto más grande sería la columna de satisfacción menor sería la de necesidades no satisfechas debido a que las personas damos mucha importancia a la salud en nuestra vida. 
...
5. Pesé a que podíamos pensar que una persona discapacitada con necesidades económicas en cosas esenciales podría tener más probabilidad de vivir en casa de un familiar que le cuide, parece ser que no existe una relación clara. Todas las comunidades presentan un porcentaje parecido de discapacitados con dificultades económicas grandes pero parece ser que las personas que viven en la casa de su cuidador lo necesitan por la gravedad de su patología y no por su nivel económico.
...
Por otro lado, no vemos una diferencia clara entre la calidad del sistema sanitario de las comunidades más rurales y las más urbanas. Las comunidades con más cantidad de pueblos (Castilla y León) presentan mayor cantidad de consultorios pero esto no implica que sea un sistema sanitario de mejor calidad. De hecho sabemos que muchos consultorios rurales presentan una atención deficitaria porque están muy poco tiempo abiertos debido a las dificultades para sufragar costes tan elevados. De hecho si revisamos las noticias estatales de los últimos días veremos datos como los siguientes:

* [Escasa inversión en centros de salud en Huesca](https://www.heraldo.es/noticias/aragon/huesca/2021/12/10/la-diputacion-critica-la-muy-escasa-inversion-de-la-dga-en-los-centros-de-salud-de-huesca-y-barbastro-1539505.html/ "Escasa inversión en centros de salud en Huesca")
    
* [Abandono de atención primaria en Campoo](https://www.eldiarioalerta.com/articulo/campoo-los-valles/denuncian-abandono-atencion-primaria-campoo/20211209130608327206.html/ "Abandono de atención primaria en Campoo")

* [Cierre consultorios en Zamora](https://www.laopiniondezamora.es/comarcas/2021/11/12/tabara-convoca-concentracion-cierre-consultorios-59430635.html/ "Cierre consultorios en Zamora")
...
![](C:\Users\usuario\Documents\INGENIERIA DE LA SALUD\3º UNI GIS\FUENTES DE DATOS BIOMEDICAS Y WEB SEMANTICAS\El precio de la discapacidad\El-precio-de-la-discapacidad\IMAGES\desigualdad_economica.png)
