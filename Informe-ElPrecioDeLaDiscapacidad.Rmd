...
Con respecto a las dificultades de gestionar el dinero, obtenemos que la media es del 0.42. Y en este caso los minimos y másximos son muy dispares, sobre todo en el caso del máximo. Tiene un valor de 1.84 y corresponde al Principado de Asturias ¿por qué?
...
Respecto a los discapacitados que no consta si viven con la persona que los cuida o que viven en otro hogar, hay una media de 15,25 en todo el estado. El mínimo se vuelve a encontrar en ¿¿¿¿NA????. Y el máximo también vuelve a darse en Andalucía, con un dato significativamente más alto que en el resto de comunidades del estado.


* **Conclusiones sobre los Cuidadores de las personas con discapacidad**

Como hemos análizado tres tipos de cuidadores, mediante la comparacíon de las medias podemos determinar que hay más discapacitados viven en el hogar de la persona que los cuida que los que viven en otros hogares, siempre teniendo encuenta el posible margen de error que podrían causar los que no constan. Cabe a destacar el caso de Andalucía, por ello le prestaremos especial atención y para ello haremos un join con el set "....", para comprobar la relación con el tipo de hogar......????
Falta por reescribir y terminar.
...
Falta_ayudas_personas_discapacidad <- read_excel("DATA/Falta de ayudas en las personas con discapacidad.xlsx")

View(Falta_ayudas_personas_discapacidad)
...
```{r}
dim(Falta_ayudas_personas_discapacidad)

class(Falta_ayudas_personas_discapacidad)

```

El set de datos es un tipo de data frame conocido como **tibble**.

Además sabemos que el tibble contiene **19 filas y 5 columnas**, esto es así porque España se compone de 17 comunidades autónomas y 2 cuidades autónomas.

También es conveniente transformar todos los nombres de la columna con "make.names" para cumplan las reglas sintácticas.

```{r}
make.names(Falta_ayudas_personas_discapacidad)

Falta_ayudas_personas_discapacidad$`Señala solo falta de ayuda técnica`

is.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de ayuda técnica`)

typeof(Falta_ayudas_personas_discapacidad$`Señala solo falta de ayuda técnica`)

is.double(as.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de ayuda técnica`))
...
```{r}
summary(Falta_ayudas_personas_discapacidad, na.rm = TRUE)

Media_tecnica <- mean(as.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de ayuda técnica`), na.rm=TRUE)

Media_tecnica

Media_asistencia <- mean(as.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de asistencia o ayuda personal`), na.rm=TRUE)

Media_asistencia

Media_ambos <- mean(as.double(Falta_ayudas_personas_discapacidad$`Señala falta de ambos tipos de ayuda`), na.rm=TRUE)

Media_ambos

Media_no <- mean(as.double(Falta_ayudas_personas_discapacidad$`No señala falta de ayuda`), na.rm=TRUE)

Media_no

Medias <- c(Media_tecnica, Media_asistencia, Media_ambos, Media_no)
