...
Por otro lado, la comunidad menos satisfecha con su entorno es Ceuta. La cual, puede deberse como en el caso anterior a diversos factores, pero también cabe destacar que puede deberse a que se trata de una de las ciudades autónomas más pequeñas del país.
...
Aunque hayamos análizado variables completamente diferentes sobre algunos aspectos de la economía de una persona discapacida, podemos sacar una conclusión generalizada de dicha economía a grandes rasgos.

Por un lado, es clara la diferneciación a nivel económico en este caso de las personas discapacitadas dependiendo de la comunidad en la que vivan, sobre todo es bastante visible la diferencia entre las comunidades situadas al sur del país y al norte.Ya que en general las del norte tiene mejor economía y menos gastos en el caso de las personas discapacitadas (con la excepción de Ceuta y Melilla).
...
```
Calculando la media de los valores de todas las comunidades de cada tipo de barreras en la participación social hemos obtenido un resultado 71.18%. Esto es, podemos afirmar que las barreras sociales que sufren las paersonas discapacitadas, teniendo en cuenta diferentes tipos de barreras, a nivel estatal estas barreras afectan al casi 3 de cada 4 personas discapacitadas.
...
Con respecto a la barrera social de salir de casa Castilla y leon  presenta el índice más bajo, mientras que Andalucía es la que tiene mayor proporción. Esto podría deberse al clima, ya que en Andalucía el calor constante que hace durantes la mayoria de los meses, podría incomodar el salir de casa. Y por lo contraría como el clima de Castilla y Leon es más cambiante y varía más la temperatura sería más agradable en este caso.
...
Con respecto a la barrera del uso del transporte la región de Murcia presenta el índice más bajo, mientras que el País Vasco y Ceuta son las que tienen mayor proporción. Esto esta completamente ligado a la calidad y vehículos adaptados que tengan en cada comunidad, por ello, podemos afrimar que tanto el País Vaco como Ceuta tendrán el transporte público más cómodo y adaptado a las personas discapacitadas.
...
library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Acceso a un empleo adecuado`)) +  geom_bar(stat="identity", position="stack", fill="brown") + ggtitle("Acceso a un empleo adecuado ") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a las barreras en el acceso a un empleo adecuado la Comunidad Foral de Navarra vuelve a presentar el índice más bajo, mientras que en este caso Ceuta es la que tiene mayor proporción. Lo que indicará que Navarra es la comunidad con menos barreras sociales para las persoans discapacitadas en cuanto a acceder a un empleo adecuado. En este caso puede deberse a diversos factores, como la facilidad a solicitar dichos trabajos, guardar plazas específicas para las personas con dificultades como estas, subenciones para que las empresas puedan ofrecer trabajos a personas con discapacidad...
...
library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Uso de internet`)) + geom_bar(stat="identity", position="stack", fill="orange") + ggtitle("Uso de internet") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a las barreras del uso de internet el País Vasco presenta el índice más bajo, mientras que Melilla es la que tiene mayor proporción. En conclusión el País vasco será la comuidad con menos barreras para que las personas con dicapacidad puedan usar el internet como cualquier otra persona. Esto puede deberse a algún tipo de ayuda que pueda tener la comunidad para evitar este tipo de dificultades en sus vidas.
...
library(tidyverse)
ggplot(data = Barreras_en_la_participación_social, aes(x = `Comunidad`, y = `Actividades de ocio y culturales`)) +  geom_bar(stat="identity", position="stack", fill="violet") + ggtitle("Actividades de ocio y culturales ") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a las barreras que pueden ocasionar las actividades de ocio y culturales Melilla presenta el índice más bajo, mientras que Andalucía es la que tiene mayor proporción. La facilidad para que las personas con discapacidad puedan realizar actividades de ocio y culturales, puede deberse a diversos factores, entre ellos, las subenciones que puedan obetener, las instalaciones adaptadas de los recintos en los que se vayan a celebrar dichas actividades ect. Y gracias a los datos, podemos saber que Melilla será quian más facilidades les de en este ámbito, disminuyendo así la barrera social.
...
Observando todos los datos y sobre todo las gráficas de cada tipo de barrera social. Hemos podido observar que a pesar de que las barreras sociales en general sean muy similares en todas las comunidades analizadas, siempre hay alguna comunidad que destaca con el bajo índice de dichas barreras, lo cual indica que si otras comunidades han podido bajar ese indice en general todas las comunidades podrían llegar al mismo nivel. Evitando así o, al menos, disminuyendo las barreras sociales con las que tienen que lidiar las personas con dicapacidad. Sabiendo que su día a día ya de por si es más compleja que el de resto de personas, las barreras sociales tendrían que ser mínimas en todas las comunidades, para no ser una barrera más en su vida diaria.
