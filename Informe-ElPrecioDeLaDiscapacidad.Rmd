...
La comunidad con mayor número de centros de salud es Cataluña que es una de las counidades con mayor densidad demográfica.
...
Este dato es muy importante para los médicos porque una persona que tiene dificultades para recordar o concentrarse tienen algún tipo de afección cognitiva donde se ve involucrado el sistema nervioso. Resulta curioso ver como Galicia presenta el índice más alto tanto de dificultades par oir como para recordar, esto podría ver porque el control de la audición también reside en el cerebro, en concreto, en el lóbulo temporal.
...
```{r}
summary(Falta_ayudas_personas_discapacidad$`Señala solo falta de ayuda técnica`, na.rm = TRUE)

Falta_ayudas_personas_discapacidad$`Comunidad`[as.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de ayuda técnica`)== min(as.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de ayuda técnica`), na.rm = TRUE)]

Falta_ayudas_personas_discapacidad$`Comunidad`[as.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de ayuda técnica`) == max(as.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de ayuda técnica`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Falta_ayudas_personas_discapacidad, aes(x = `Comunidad`, y = `Señala solo falta de ayuda técnica`)) +  geom_bar(stat="identity", position="stack", fill="pink") + ggtitle("Señala solo falta de ayuda técnica") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a la falta de ayudas técnicas Castilla la Macha presenta el índice más bajo, mientras que Murcia es la que tiene mayor proporción.

```{r}
summary(Falta_ayudas_personas_discapacidad$`Señala solo falta de asistencia o ayuda personal`, na.rm = TRUE)

Falta_ayudas_personas_discapacidad$`Comunidad`[as.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de asistencia o ayuda personal`)== min(as.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de asistencia o ayuda personal`), na.rm = TRUE)]

Falta_ayudas_personas_discapacidad$`Comunidad`[as.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de asistencia o ayuda personal`) == max(as.double(Falta_ayudas_personas_discapacidad$`Señala solo falta de asistencia o ayuda personal`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Falta_ayudas_personas_discapacidad, aes(x = `Comunidad`, y = `Señala solo falta de asistencia o ayuda personal`)) +  geom_bar(stat="identity", position="stack", fill="green") + ggtitle("Señala solo falta de asistencia o ayuda personal") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a la falta de ayudas asistenciales País Vasco presenta el índice más bajo, mientras que Islas Baleares es la que tiene mayor proporción.
...
```{r}
summary(Falta_ayudas_personas_discapacidad$`Señala falta de ambos tipos de ayuda`, na.rm = TRUE)

Falta_ayudas_personas_discapacidad$`Comunidad`[as.double(Falta_ayudas_personas_discapacidad$`Señala falta de ambos tipos de ayuda`)== min(as.double(Falta_ayudas_personas_discapacidad$`Señala falta de ambos tipos de ayuda`), na.rm = TRUE)]

Falta_ayudas_personas_discapacidad$`Comunidad`[as.double(Falta_ayudas_personas_discapacidad$`Señala falta de ambos tipos de ayuda`) == max(as.double(Falta_ayudas_personas_discapacidad$`Señala falta de ambos tipos de ayuda`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Falta_ayudas_personas_discapacidad, aes(x = `Comunidad`, y = `Señala falta de ambos tipos de ayuda`)) +  geom_bar(stat="identity", position="stack", fill="blue") + ggtitle("Señala falta de ambos tipos de ayuda") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```

Con respecto a la falta de ayudas asistenciales Navarra presenta el índice más bajo, mientras que Castilla la Mancha es la que tiene mayor proporción. Resulta sorprendente ver como Castilla la Mancha presentaba el índice de ayudas técnicas más bajo, mientras que tienen la mayor proporción de la necesidad de ambos tipos de ayuda. Nos imaginamos que esto refleja que el sistema sanitario de Castilla la Mancha tiene un problema con respecto a las ayudas asistenciales que ofrece a los discapacitados.

```{r}
summary(Falta_ayudas_personas_discapacidad$`No señala falta de ayuda`, na.rm = TRUE)

Falta_ayudas_personas_discapacidad$`Comunidad`[as.double(Falta_ayudas_personas_discapacidad$`No señala falta de ayuda`)== min(as.double(Falta_ayudas_personas_discapacidad$`No señala falta de ayuda`), na.rm = TRUE)]

Falta_ayudas_personas_discapacidad$`Comunidad`[as.double(Falta_ayudas_personas_discapacidad$`No señala falta de ayuda`) == max(as.double(Falta_ayudas_personas_discapacidad$`No señala falta de ayuda`), na.rm = TRUE)]

library(tidyverse)
ggplot(data = Falta_ayudas_personas_discapacidad, aes(x = `Comunidad`, y = `No señala falta de ayuda`)) +  geom_bar(stat="identity", position="stack", fill="red") + ggtitle("No señala falta de ayuda") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

```
Con respecto a las comunidades que no señalan falta de ayudas Murcia presenta el índice más bajo, mientras que Canarias es la que tiene mayor proporción.
...
```{r}
summary(as.double(Falta_ayudas_personas_discapacidad$`Señala falta de ambos tipos de ayuda`), na.rm = TRUE)

summary(as.double(Falta_ayudas_personas_discapacidad$`No señala falta de ayuda`), na.rm = TRUE)

Falta_ayudas_personas_discapacidad$`Señala falta de ambos tipos de ayuda`[Falta_ayudas_personas_discapacidad$`Comunidad`== "Castilla y León"]

Falta_ayudas_personas_discapacidad$`No señala falta de ayuda`[Falta_ayudas_personas_discapacidad$`Comunidad`== "Castilla y León"]
```
...
Falta_ayudas_personas_discapacidad$`Señala falta de ambos tipos de ayuda`[Falta_ayudas_personas_discapacidad$`Comunidad`== "Aragón"]

Falta_ayudas_personas_discapacidad$`No señala falta de ayuda`[Falta_ayudas_personas_discapacidad$`Comunidad`== "Aragón"]
```
...
Falta_ayudas_personas_discapacidad$`Señala falta de ambos tipos de ayuda`[Falta_ayudas_personas_discapacidad$`Comunidad`== "Murcia, Región de"]

Falta_ayudas_personas_discapacidad$`No señala falta de ayuda`[Falta_ayudas_personas_discapacidad$`Comunidad`== "Murcia, Región de"]
```
...
Falta_ayudas_personas_discapacidad$`Señala falta de ambos tipos de ayuda`[Falta_ayudas_personas_discapacidad$`Comunidad`== "Madrid, Comunidad de"]

Falta_ayudas_personas_discapacidad$`No señala falta de ayuda`[Falta_ayudas_personas_discapacidad$`Comunidad`== "Madrid, Comunidad de"]
``` 
